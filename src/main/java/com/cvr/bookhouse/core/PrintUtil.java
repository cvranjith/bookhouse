...
public final class PrintUtil {
    ...
    public static <T> String renderTable(
            List<String> headers,          // e.g. List.of("Book", "Total Copies", "Available Copies")
            List<T> items,
            Function<T, List<String>> rowMapper
    ) {
        if (items == null || items.isEmpty()) return "(no rows)";

        // prepend serial header
        List<String> hdr = new ArrayList<>(headers.size() + 1);
        hdr.add("sl");
        hdr.addAll(headers);

        // build rows and inject serial
        ...
        for (List<String> r : rows) {
            while (r.size() < colCount) r.add("");
            if (r.size() > colCount) r.subList(colCount, r.size()).clear();
        }

        // compute widths (capped at MAX_COL_WIDTH)
...
private static String formatSeparator(int[] widths) {
    ...
    // first column
    for (int k = 0; k < widths[0]; k++) sb.append('-');

    // subsequent columns: "-+-" matches " | "
    for (int c = 1; c < widths.length; c++) {
        sb.append("-+-");
        for (int k = 0; k < widths[c]; k++) sb.append('-');
    }
    return sb.toString();
}
